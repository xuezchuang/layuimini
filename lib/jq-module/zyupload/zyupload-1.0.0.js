...
$.Jcrop = function(obj,opt)
{
	...
	if (!('trackDocument' in opt)) {
		var isIE = /*@cc_on!@*/false || !!document.documentMode; // 检测是否为 IE 浏览器
		opt.trackDocument = !isIE; // 如果是 IE，则设置为 false；否则为 true

		// 检查是否为 IE 8
		var isIE8 = document.documentMode && document.documentMode === 8;
		if (isIE8) {
			opt.trackDocument = true; // 如果是 IE 8，则设置为 true
		}
	}

	if (!('keySupport' in opt)) {
		// 之前用 $.browser.msie 检查 IE，现在使用原生 JavaScript 检测
		var isIE = /*@cc_on!@*/false || !!document.documentMode;
		opt.keySupport = !isIE; // 如果是 IE，则设置为 false；否则为 true
	}
	...
	$.fn.zyPopup = function(options,param){
		...
		return this.each(function(){
			...
			this.openPopup = function(){
				...
				image.on('load', render).attr('src', para.src);
