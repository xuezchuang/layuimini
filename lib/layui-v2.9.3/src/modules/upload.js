...
layui.define(['lay', 'layer'], function(exports){
  ...
  Class.prototype.upload = function(files, type){
      ...
      var request = function(sets){
        ...
        var opts = {
          ...
          success: function(res){ // 成功回调
            ...
            let timeElapsed = (currentTime - donestartTime) / 1000;
            let speed = 0;
            if (timeElapsed > 0) {
              speed = totalsize / timeElapsed; // 计算速度（单位取决于loaded的单位）
            }
            done(sets.index, res,speed);
            allDone(sets.index);
          ...
          opts.xhr = function(){
            ...
            xhr.upload.addEventListener("progress", function (obj) {
              if(obj.lengthComputable){
                ...
                let speed = 0;
                if (timeElapsed > 0) {
                  speed = (obj.loaded - previousLoaded) / timeElapsed; // 计算速度（单位取决于loaded的单位）
                }
                previousLoaded = obj.loaded;
                startTime = currentTime;
                options.progress(percent, (options.item ? options.item[0] : options.elem[0]) , obj, sets.index,speed);
                totalsize = obj.total;
              }
            });
            return xhr;
    ...
    // 统一回调
    var done = function(index, res,speed){
